...
/**
 * Detects user-defined payload strings in HTTP request content.
 * Categories are fully customizable — pentesters can add their own
 * beyond the defaults (XSS, SQLi, etc.).
 */
public class AttackDetector {

    private static final List<String> DEFAULT_CATEGORIES = Arrays.asList(
        "XSS", "SQLi", "Path Traversal", "CMDi", "SSTI", "SSRF", "XXE"
    );
    ...
    /**
     * Remove a custom category and its payloads.
    ...
    /**
     * Detect user-defined payloads in request content.
     * Returns {category: AttackPattern} for each category matched.
     */
    ...
    private void loadPayloads() {
        ...
        try (Reader reader = new InputStreamReader(
                new FileInputStream(toLoad), StandardCharsets.UTF_8)) {
        ...
        try {
            ...
            for (String cat : categories) {
                data.put(cat, new ArrayList<>(
                    payloads.getOrDefault(cat, Collections.emptySet())));
            }
            ...
            java.nio.file.Files.move(tmp.toPath(), payloadFile.toPath(),
