...
/**
 * Converts dynamic URL path segments into reusable endpoint groups. E.g. /users/123 and /users/456
 * both become /users/{id}.
 *
 * <p>When {@code aggressive} is true (e.g. Intruder traffic), even single-digit numeric segments
 * are normalized to {id} so payload iterations collapse into one endpoint row.
 */
public class PathNormalizer {
  ...
  private static final Pattern HEX_TOKEN_RE =
  ...
  private static final Pattern SWAGGER_PARAM_RE = Pattern.compile("^\\{[^}]+\\}$");
  ...
  public static String normalizeSegment(String segment, boolean aggressive) {
    ...
    if (ALPHANUM_TOKEN_RE.matcher(segment).matches() && HAS_DIGIT.matcher(segment).find()) {
      return "{token}";
    }
    return segment;
  }
  ...
  /**
   * Normalize a Swagger/OpenAPI path so it matches captured traffic paths. Converts {paramName}
   * placeholders to {id} so they align with the numeric normalization applied to real traffic (e.g.
   * /users/123 → /users/{id}).
   */
  public static String normalizeSwaggerPath(String path) {
    if (path == null || path.isEmpty()) return "/";
    int qi = path.indexOf('?');
    if (qi >= 0) path = path.substring(0, qi);
    ...
    for (int i = 0; i < segments.length; i++) {
      ...
      if (SWAGGER_PARAM_RE.matcher(seg).matches()) {
        // Map {uuid} and {*uuid*} variants to {uuid}, everything else to {id}
        String inner = seg.substring(1, seg.length() - 1).toLowerCase();
        ...
        sb.append(normalizeSegment(seg));
      }
    }
    String result = sb.toString();
    return result.isEmpty() ? "/" : result;
