...
/**
 * Extracts TrafficRecord from Burp HTTP messages and aggregates
 * records into EndpointRows for display.
 */
public class TrafficParser {
    ...
    public static String getToolName(ToolType tool) {
        ...
        switch (tool) {
            ...
            case SCANNER:   return "Active Scan";
            case EXTENSIONS: return "Extender";
            case TARGET:    return "Target";
            case SEQUENCER: return "Sequencer";
            case DECODER:   return "Decoder";
            case COMPARER:  return "Comparer";
    ...
    private TrafficRecord buildRecord(HttpRequest request, HttpResponse response,
                                       ToolType tool) {
        ...
        try {
                ...
                for (ParsedHttpParameter param : request.parameters()) {
                    ...
                    if (ptype == HttpParameterType.URL || ptype == HttpParameterType.BODY) {
                        ...
                        if (name != null && !name.isEmpty()) {
                            ...
                            try { paramValues.put(name, param.value()); }
                            catch (Exception ignored) {}
                        }
                    }
                }
            ...
            if (tool != ToolType.PROXY && tool != ToolType.TARGET) {
                try {
                    String reqStr = new String(request.toByteArray().getBytes(),
    ...
    public static AggregationResult aggregate(List<TrafficRecord> records,
                                               Map<String, String> notesStore,
                                               Map<String, String> tagsStore) {
        ...
        for (AggEntry entry : endpointMap.values()) {
            ...
            String depth = classifyDepth(entry.toolFlags, entry.hasEditedProxy,
                                          entry.requestCount);
            ...
            row.setPriority(classifyPriority(methodsList, paramsList, authStates,
                                             entry.statusCodes, depth));
        ...
        rows.sort(Comparator.comparing(EndpointRow::getHost)
                            .thenComparing(r -> -r.getRequestCount()));
    ...
    public static String classifyDepth(Set<ToolType> toolFlags, boolean hasEditedProxy,
                                        int requestCount) {
        boolean hasFuzz = toolFlags.contains(ToolType.INTRUDER)
                       || toolFlags.contains(ToolType.SCANNER);
        boolean hasManual = toolFlags.contains(ToolType.REPEATER)
    ...
    public static String classifyPriority(List<String> methods, List<String> queryParams,
    ...
    public static class Summary {
        ...
        public int getUniqueHosts() { return uniqueHosts; }
        public void setUniqueHosts(int v) { this.uniqueHosts = v; }
        public int getUniqueRawUrls() { return uniqueRawUrls; }
        public void setUniqueRawUrls(int v) { this.uniqueRawUrls = v; }
        public int getUniqueEndpoints() { return uniqueEndpoints; }
        public void setUniqueEndpoints(int v) { this.uniqueEndpoints = v; }
        public Map<String, Integer> getStatusCounts() { return statusCounts; }
        public void setStatusCounts(Map<String, Integer> v) { this.statusCounts = v; }
    }
    ...
    public static class AggregationResult {
        ...
        public List<EndpointRow> getEndpointRows() { return endpointRows; }
    }
}
